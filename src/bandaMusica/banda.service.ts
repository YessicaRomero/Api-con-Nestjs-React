...
export class BandaService {
  ...
  async getBandasById(id: number): Promise<iBanda> {
    try {
      ...
      throw new NotFoundException(`La banda con id ${id} no existe`);
    }
  }
  ...
  async getBandasByGener(genero: string): Promise<iBanda> {
    try {
      ...
      throw new NotFoundException(`la Banda con ${genero} no existe`);
    }
  }
  ...
  async getBandasByNombre(nombre: string): Promise<iBanda> {
    try {
      ...
      throw new NotFoundException(`la Banda ${nombre} no existe`);
    }
  }
  ...
  async updateBandaById(id: number, body: BandaDto): Promise<void> {
    ...
    if (!Object.keys(isBanda).length) return;
